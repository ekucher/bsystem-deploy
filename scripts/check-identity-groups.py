...
A group name decides access, and it is written three times in three places
that never meet:
  ...
  3. the Integration Core's RBAC seed migration what actually grants a role
...
the mock, so it compares (2) against (3) and never reads (1) at all. A typo in
...
This script compares the two lists this repository owns, and the third as well
when a sibling checkout of bsystem-integration-core is present.
"""
